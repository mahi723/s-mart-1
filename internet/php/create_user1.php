...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	try {
		...
		if ($success) {
			...
			if ($success) {
					// add customer_id as image_id for matching profile images
				$result = $query->get_result();
				$customer = $result->fetch_object();
				$customerId = $customer->customer_id;
				$imageQuery = $connection->prepare('UPDATE customers SET image_id = ?');
				$imageQuery->bind_param('i', $customerId);
				$imageId = $imageQuery->execute();
				// header('Location: ../');
				var_dump($customerId);
			}
			else {
